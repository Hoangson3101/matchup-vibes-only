      ...
      <div className="container px-4 pt-4">
        <div className="flex flex-col items-center">
          <div className="w-32 h-32 rounded-full overflow-hidden mb-4">
            <img 
              src={user.avatar || 'https://via.placeholder.com/300?text=Avatar'} 
              alt={user.name} 
          ...
          <div className="mt-8 w-full">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              {Array(Math.max(0, 6 - (user.photos?.length || 0))).fill(0).map((_, index) => (
                <div key={`empty-${index}`} className="aspect-square rounded-lg bg-muted flex items-center justify-center">
                  {editable && <Pencil size={20} className="text-muted-foreground/50" />}
                </div>
              ))}
            </div>
          </div>
