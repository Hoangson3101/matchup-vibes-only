...
const Payment = () => {
  ...
  const handlePayment = async () => {
    ...
    try {
      // In a real app, you would integrate with VNPay API here
      const finalPrice = paymentData.price - discountAmount;
      
      // Simulate VNPay payment process
      const vnpayUrl = `https://sandbox.vnpayment.vn/paymentv2/vpcpay.html?vnp_Version=2.1.0&vnp_Command=pay&vnp_TmnCode=DEMO&vnp_Amount=${finalPrice * 100}&vnp_CurrCode=VND&vnp_TxnRef=${Date.now()}&vnp_OrderInfo=Nap kim cuong MatchUp&vnp_OrderType=other&vnp_Locale=vn&vnp_ReturnUrl=${window.location.origin}/payment-success&vnp_IpAddr=127.0.0.1&vnp_CreateDate=${new Date().toISOString().replace(/[-:]/g, '').split('.')[0]}`;
